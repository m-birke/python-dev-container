...
# docker
alias docker_rm_all_containers='docker container ls -a -q | xargs -I % docker container rm %'
alias docker_rm_all_images='docker image ls -a -q | xargs -I % docker image rm %'
...
mongo() {
    user=$(jq .user .token/mongodb_admin.json)
    pw=$(jq .pw .token/mongodb_admin.json)
    mongosh mongodb://${user:1:-1}:${pw:1:-1}@rbglv0071.rbg.infineon.com:27017/?authSource=kaisdm&replicaSet=rs&readPreference=primary&appname=MongoDB%20Compass&ssl=false
}
